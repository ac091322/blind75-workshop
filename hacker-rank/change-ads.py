...
def changeAds(base10):
    # max number of bits used by current system
    # will usually return a number that is just shy of the maximum value
    system_bit_length = sys.maxsize.bit_length()  # 63 on a 64 bit system
    # print("system_bit_length:", system_bit_length)
    ...
    # find the highest order 1 bit (the first 1) and return its index
    # this and related code would be pointless unless specifically assigned leading 0s (but wouldn't hurt)
    first_1_index = binary_str.find("1")  # always 0
    # print("first_1_index:", first_1_index)
