...
def max_area(heights):
    ...
    # special strings like "inf" and "nan" are recognized by the float() function
    max_area = float("-inf")  # equivalent of -Infinity in js

    while left_index < right_index:
        left_height = heights[left_index]
        right_height = heights[right_index]
        container_width = right_index - left_index
        area = container_width * min(left_height, right_height)
...
print(max_area([2, 3, 2]))  # output: 4
print(max_area([1, 1]))  # output: 1
print(max_area([1, 6, 8, 3, 9, 1]))  # output: 18
