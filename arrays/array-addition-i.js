...
function arrayAddition(arr) {
    // find the largest number and remove it from the array
    ...
    function getCombinations(arr, size) {
        const result = [];
        function combinations(start, current) {
    ...
    // check all combinations of numbers in the array
    for (let r = 1; r <= arr.length; r++) {
        const combos = getCombinations(arr, r);
        for (let combo of combos) {
            if (combo.reduce((acc, num) => acc + num, 0) === largestNum) {
                return true;
            }
        }
    }
...
console.log(arrayAddition([5, 10, 15, 20]));  // output: true
console.log(arrayAddition([1, 2, 3, 6]));  // output: true
console.log(arrayAddition([1, 2, 3, 7]));  // output: false
console.log(arrayAddition([10, 5, 3, 2, 8]));  // output: true
console.log(arrayAddition([100, 250, 500, 1000, 1500, 2000, 7000]));  // output: false
