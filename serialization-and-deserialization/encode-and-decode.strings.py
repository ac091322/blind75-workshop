...
def encode(strs):
    ...
    for substring in strs:
        # the length will be used to decode the string by finding the length after the delimiter
        encoded += str(len(substring)) + "#" + substring
...
def decode(s):
    ...
    while i < len(s):
        ...
        while s[j] != "#":
            j += 1  # move j forward until the # is found
            # it does not enter into the next line until the # is found

        # slice from i to j, which will only slice a string number, then convert it to an integer
        length = int(s[i:j])
...
print(encoded)  # encoded: "4#neet4#code4#love3#you"
decoded = decode(encoded)
print(decoded)  # decoded: ["neet", "code", "love", "you"]
